#!/usr/bin/env bash
# Offscreen-render Main.qml (kith_ui) with a mock `logos` backend -> screenshots in shots/.
# Catches runtime/layout bugs a nix build can't (nix build only proves the files copy

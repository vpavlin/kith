...
// from scala_identity.hpp (itself ported from the proven qaku implementation). Same
// byte-parity recipe, so a Kith contact.set signed on desktop verifies unchanged on
// a future mobile port:
//   address(pub)     = "0x" + hex(sha256(pub_compressed_33B))[24:64]   (last 20 bytes)
//   canonicalMessage = "kith-sig-v1|"+type+"|"+wall+"|"+ctr+"|"+dev+"|"+id+"|"+cjson(payload)
...
// This is the AUTHENTICITY layer (who authored a contact.set/contact.del) - orthogonal
// to any future confidentiality seal on a shared book. It is what makes the
// loamIdentity.address on a contact THE SAME key that can later verify that person's
// own writes elsewhere (kith ADR 0002 / 0003): one key does double duty.
//
// This local device key is a FALLBACK signer only - the primary path is authoring
// through loam_core's identity service (see kith_impl.cpp's mkEvent, modeled 1:1 on
// scala_impl.cpp's mkEvent), which signs with the book's BOUND loam identity so a
// book's owner is a real loam identity, not an ad hoc per-app device key. Keycard
// support (loam ADR 0004-style card-signed writes) is explicitly OUT of scope for
// this v1 core - see kith docs/adr/0008; add it the way scala_impl.cpp's
// authorEvent/authorAndPublish/enrollKeycard do, when it's time.
#include <string>
#include <vector>
#include <algorithm>
#include <cstdio>
...
// Adopt the shared envelope into kith:: (kith_engine.hpp does the same; a repeated
// using-declaration in the same namespace is allowed). This header may be included
// before kith_engine's own adoption, so it must not depend on it.
...
// Canonical JSON: sorted object keys, compact, integer numbers as-is. Kith payloads
// are strings/ints/bools/arrays/objects - no floats.
...
inline std::string canonicalMessage(const Event& e) {
    ...
    return "kith-sig-v1|" + e.type + "|" + std::to_string(e.hlc.wall) + "|"
...
// backward compat by callers that choose to, but never treated as authenticated.

// Per-device signing identity + event authentication (secp256k1). BYTE-PARITY with
// the desktop core (src/kith_identity.hpp) — a phone-signed event verifies on the
// desktop and vice-versa. This is the AUTHENTICITY layer (who authored a
// contact.set/contact.del), orthogonal to the AES-GCM book seal (confidentiality).
// It is what makes loamIdentity.address on a contact the SAME key that can later
// verify that person's own writes elsewhere (kith ADR 0002 / 0003).
//
...
//
// This is the mobile mirror of the desktop's LOCAL DEVICE KEY fallback signer
// (kith_identity.hpp's identityFromPriv/generateIdentity path). The desktop's
// PRIMARY path signs through loam_core's bound identity per book; the phone has
// no loam_core, so every event here is signed with this one device key — same as
// kith_impl.cpp falls back to when a book has no loam_core-bound identity yet.
import * as SecureStore from "expo-secure-store";
import * as Crypto from "expo-crypto";
// @noble/curves v1.x — v1 `sign()` signs the hash directly (no prehash) and returns
// a Signature (`.toCompactHex()`); cross-verifies with the desktop OpenSSL core.
...
export async function getIdentity(): Promise<Identity> {
  ...
  let hp = await SecureStore.getItemAsync("kith-identity-key");
  if (!hp) { hp = hex(freshPriv()); await SecureStore.setItemAsync("kith-identity-key", hp); }
...
export function canonicalMessage(ev: any): string {
  ...
  return "kith-sig-v1|" + ev.type + "|" + wall + "|" + ctr + "|" + dev + "|" + ev.id + "|" + cjson(ev.payload || {});
}

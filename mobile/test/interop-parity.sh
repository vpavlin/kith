...
set -euo pipefail
cd "$(dirname "$0")/.."
gcc -std=c11 test/interop_open.c -o /tmp/kith_interop_open -lssl -lcrypto
KEY="3190cb26-10e8-4dfa-a8f6-20decd5c0931f5bd9779-2e14-44c7-ae29-b9cd1cd2400d"
for PLAIN in '{"v":1,"type":"contact.set","hello":"from the phone"}' 'x' 'a longer payload with, commas and "quotes" and unicode ☕'; do
  OUT=$(node --experimental-strip-types test/interop_seal.mjs "$KEY" "$PLAIN" 2>/dev/null | /tmp/kith_interop_open)

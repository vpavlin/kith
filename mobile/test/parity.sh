...
# desktop engine (src/kith_engine.hpp) MUST fold the same event log to the same
# book state — else desktop and phone diverge on the same channel.
set -euo pipefail
cd "$(dirname "$0")"
...
# OpenSSL — the fold VERIFIES signatures (kith_identity.hpp → libcrypto), and always-require means

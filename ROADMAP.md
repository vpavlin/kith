...
| 5 | **Android app** — React-Native/Expo app on loam-transport + logos-sync, with the contact fold reimplemented in TS in lockstep with `kith_engine.hpp` (ADR 0006), TS app + debug APK build; C++/TS fold PARITY test passes; **pending: release keystore, on-device test, F-Droid publish** | Sonnet | ◑ builds (debug APK) |
